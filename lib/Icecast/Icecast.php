    ...
    function getStatus() {
        ...
        if(count($temp_array)) {
			...
			/*if ($_GET['server'] == 'oxyradio') {
				...
				$this->radio_info['most_listeners'] = "";
				$this->radio_info['genre'] = $temp_array[6];
				$this->radio_info['url'] = $temp_array[7];

				if(isset($temp_array[8])) {
					$x = explode(" - ",$temp_array[8]);
					if (count($x)>2) {
						$this->radio_info['now_playing']['artist'] = $x[0]."-".$x[1];
						$this->radio_info['now_playing']['track'] = $x[2];
					} else {
